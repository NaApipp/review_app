...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(async () => {
    try {
      await fetch("/api/anotator-laptop/auth/logout", { method: "POST" });
    } catch {
      // no-op
    }
    ...
    if (pathname !== "/login-anotator-laptop") router.replace("/login-anotator-laptop");
  }, [pathname, router]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/anotator-laptop/auth/me", {
