...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(async () => {
    try {
      await fetch("/api/anotator-fashion/auth/logout", { method: "POST" });
    } catch {
      // no-op
    }
    ...
    if (pathname !== "/login-anotator-fashion") router.replace("/login-anotator-fashion");
  }, [pathname, router]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/anotator-fashion/auth/me", {
