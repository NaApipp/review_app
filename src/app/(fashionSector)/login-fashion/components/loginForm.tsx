...
export default function LoginForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/fashion/auth/login", {
        method: "POST",
      ...
      router.replace("/beranda-fashion");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="relative hidden lg:flex flex-col justify-between p-10 bg-gradient-to-br from-[#071c2f] to-[#0c2f4f] text-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-lg">Sector Fashion</span>
          </div>
        ...
        <div className="flex items-center justify-center px-6 bg bg-gradient-to-br from-[#0F2854] to-[#1C4D8D]">
          <div className="w-full max-w-md">
            {/* Header */}
            <div className="mb-8">
              <h2 className="text-3xl font-bold mt-2">Sector Fashion Login</h2>
            ...
            <Link href={"/fashion-sector"} className="md:hidden mt-4 flex flex-row w-max rounded-2xl items-center p-2 text-sm text-white/70 bg-blue-500 hover:bg-blue-600 transition-colors">
            <svg
              className="w-8 h-8 text-white dark:text-white"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
