...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(async () => {
    try {
      await fetch("/api/anotator-user2/anotator-fashion/auth/logout", { method: "POST" });
    } catch {
      // no-op
    }
    ...
    if (pathname !== "/login-anotator-fashion-v2") router.replace("/login-anotator-fashion-v2");
  }, [pathname, router]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/anotator-user2/anotator-fashion/auth/me", {
